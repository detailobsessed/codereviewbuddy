...
def _parse_comma_logins(v: str | list[str]) -> list[str]:
    """Accept ``alice,bob`` as a comma-separated string or an already-parsed list."""
    if isinstance(v, str):
...
class Config(BaseModel):
    ...
    owner_logins: list[str] = Field(
        ...
        description="GitHub usernames considered 'ours' for triage filtering (comma-separated in env, e.g. 'alice,bob')",
    )
...
def load_config() -> Config:
    ...
    class _EnvConfig(BaseSettings):
        ...
        owner_logins: str = Field(default="")

    env = _EnvConfig()
    config = Config(
        pr_descriptions=env.pr_descriptions,
        self_improvement=env.self_improvement,
        owner_logins=_parse_comma_logins(env.owner_logins),
    )
